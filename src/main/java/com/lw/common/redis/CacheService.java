...
/*`CacheService` 类是一个用于与 Redis 数据存储交互的实用类，使用了 Spring Data Redis 框架。以下是其功能的简要概述：

        1. **列表操作**：
        - **左弹出**：移除并返回列表的第一个元素。
        - **阻塞左弹出**：移除并返回列表的第一个元素，阻塞直到有元素可用或达到超时时间。
        - **右弹出**：移除并返回列表的最后一个元素。
        - **阻塞右弹出**：移除并返回列表的最后一个元素，阻塞直到有元素可用或达到超时时间。
        - **右弹出并左推**：移除列表的最后一个元素，并将其添加到另一个列表的头部。
        - **阻塞右弹出并左推**：移除列表的最后一个元素，并将其添加到另一个列表的头部，阻塞直到有元素可用或达到超时时间。
        - **移除元素**：从列表中移除值等于指定值的元素。
        - **列表裁剪**：裁剪列表，只保留指定范围内的元素。

        2. **集合操作**：
        - **获取键的集合**：通过模式匹配获取符合条件的键的集合。

        3. **管道技术**：
        - **批量右推**：使用管道技术批量将元素添加到列表，提高性能。
        - **刷新操作**：使用管道技术刷新数据，包括批量添加元素和移除元素。

        该类通过封装 Redis 的常用操作，为应用提供了简便的缓存操作接口，从而提高了数据访问的性能和效率。*/
@Component
